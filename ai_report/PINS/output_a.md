# PINS — Task A：价格层（daily）
> 更新: 2026-08-08  现价 $23.68（Aug 7 close）
...
| 口径 | 值 | 来源 |
|------|-----|------|
| 现价 | $23.68 | CSV（Aug 7 close） |
| 合理价 | $7.99 | from D（$0.34 × 23.5x） |
| 满仓目标 | $5.35 | from D（$7.99 × 0.67） |
| **安全边际** | **−196%** | 1 − 23.68/7.99 |

> 现价远高于合理价 → **买贵了**。三档敏感性（$6.29-$9.01）均远低于现价。DCF FCF−SBC $12.27 仍 < 现价 → 即使 realistic DCF 口径也偏贵。

## 结论

入池: ✓（1/7 + P/S ~1st 分位 + FCF yield 9.5%）。但 P/E 67.7x（GAAP 真相，SBC 压低）+ 安全边际 −196% → **买贵了**。

PINS 的核心矛盾：FCF $1.28B（yield 9.5%）看似便宜，但 SBC $1.02B 吃 80% → GAAP EPS $0.34 → P/E 67.7x。回购 $3.10B 真缩股（698→642M）但债务融资不可持续。框架立场：SBC 是真实成本 → 用 GAAP EPS → 买贵了。
