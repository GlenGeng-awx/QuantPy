...
> 牛口径（$89.93）安全边际 = 1 − $101.76/$89.93 = −13.2% → 即便用 tool EPS + g=8% 仍买贵了。
>
> **旧 output 口径**（v3.1 $5.60 × g=8% × PE=16.5 = $92.40）→ 安全边际 −4%（接近合理价）。但 v3.1 $5.60 已失效（TTM CSV COGS 错误 → GMDrop 假阳性 → v3.1 = −$0.66）。现用 GAAP $4.85 → 合理价降至 $75.18 → 安全边际 −35%。
