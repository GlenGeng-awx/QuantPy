...
for stock_name in STOCK_NAMES_TIER_0:
    ...
    for (start_date, end_date, interval) in get_period(stock_name):
        ...
        # close + volume reg + sr
        de.build_graph(enable_close_price=True, enable_volume_reg=True, enable_sr=True)
        de.display()

        # close + volume reg + wave 3rd
        de.build_graph(enable_close_price=True, enable_volume_reg=True, enable_wave=True)
        de.display()
        ...
        # close + volume reg + ema 10/20
        de.build_graph(enable_close_price=True, enable_volume_reg=True, enable_ema=True)
        de.display()

        # close + volume reg + bb
        de.build_graph(enable_close_price=True, enable_volume_reg=True, enable_bband=True)
        de.display()
